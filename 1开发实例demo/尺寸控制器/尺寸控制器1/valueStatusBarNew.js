...
var valuestatusBar = function (obj) {
        ...
        switch (event.type) {
            case "touchstart":
              //  alert(event.touches[0].clientX);
                mouseCurrentX = event.touches[0].clientX;
                break;
            case "touchmove":
            case "touchend":
                mouseCurrentX = event.changedTouches[0].clientX;
                break;
        }
    ...
    function getFinalValueByStep(value) {
        var stepCount = Math.floor(value / datastep); //步数
