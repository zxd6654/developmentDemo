...
var valuestatusBar = function (obj) {
    ...
    function setIntroduceValue(introValueArr) {
        ...
        var radios=containner.getElementsByTagName("input");

        for (var d = radios.length - 1; d >= 0; d--) {
            var curdiv = radios[d];
        ...
        for (var i = 0, len = introValueArr.length; i < len; i++) {
            ...
            input.type="radio";
            input.name="date";
            input.className = "levelpointer";
            /**
             *  left为左上角的位置，
             *  添加滑块宽度的一半调节到滑块的中心显示
             *  位置+对应滑块的微调+ 对应步长的微调，调到步长中心
             */
            input.style.left = ( left + moveBlockWidth / 2 + getDataStepLength() / 2-7.5) + "px";
            var span = document.createElement("span");
            span.innerHTML = introValue;
            span.className = "introductorVal";
            span.style.left = (left + getDataStepLength() / 2) + "px";
            containner.appendChild(input);
