...
var valuestatusBar = function (obj) {
    ...
    var pMouseStart = document.getElementById("mouseStart");
    ...
    function getMousePos(e) {    /*鼠标横坐标的位置*/
        ...
        switch (event.type) {
            case "mousedown":
                var mouseStart = mouseCurrentX;
                // console.log(mouseStart);
                pMouseStart.innerHTML = mouseStart;
                break;
            case "mouseup":
                var mouseEnd = mouseCurrentX;
        ...
        return {
            ...
            mouseStartPos: mouseStart,
            mouseEndPos: mouseEnd
        };
    }

    //将鼠标在页面的坐标转化在滑动容器内的坐标
    function transMouseX(e) {  /*返回滑块的坐标*/
        var pageMouse = getMousePos(e).mouseMoveX;
        /*鼠标横坐标的位置*/
    ...
    function mouseUpDealer() {
        ...
        // setSlideShower(finalOutPutValue);
        // slideEndMoveBlock(finalOutPutValue);
    }

    //鼠标移动处理函数
    var finalOutPutValue;
    /*800*/
    var oldOutPutValue = datadefault;
    ...
    function slideEndMoveBlock(value) {
       if(typeof  slideEndFunction==="function"){
           ...
           var showerParent = moveBlock.parentNode;
           var shower = showerParent.getElementsByTagName("span")[0];
           var mouseStartPos = pMouseStart.innerHTML;
           var mouseEndPos = getMousePos().mouseEndPos;
           console.log(mouseStartPos);
           console.log(mouseEndPos);
           for (var i = 0; i < introValueArr.length; i++) {
               //获取推荐值指示标签的位置
               // var left = getIntroductorPos(introValueArr[i]),
               //     leftNext = getIntroductorPos(introValueArr[i + 1]),
               //     div = document.getElementsByClassName("levelpointer")[i],
               //     divNext = document.getElementsByClassName("levelpointer")[i + 1],
               //     divWidth = div.style.left = ( left + moveBlockWidth / 2 + getDataStepLength() / 2);
               // /*获取当前推荐值的位置*/
               // if (i + 1 <= introValueArr.length - 1) {
               //     var divWidthNext = divNext.style.left = ( leftNext + moveBlockWidth / 2 + getDataStepLength() / 2);
               //     //    获取下一个推荐值的位置
               // }
               // if (i === 0) {
               //     var leftOne = getIntroductorPos(introValueArr[0]),
               //         divOne = document.getElementsByClassName("levelpointer")[0],
               //         divWidthOne = divOne.style.left = ( leftOne + moveBlockWidth / 2 + getDataStepLength() / 2);
               // }
               if (value > introValueArr[i] && value < introValueArr[i + 1]) {
                   if (mouseStartPos < mouseEndPos) {
                       data.value= introValueArr[i + 1];
                       // moveBlock.style.left = (divWidthNext - moveBlockWidth / 2) + "px";
                       // console.log(i+1);
                       // setMoveBlockByValue(data.value);
                       // inputValueTransfer(data.value);//外部传入3d的函数处理
                       setSlideShower(introValueArr[i + 1]);
                       setMoveBlockByValue(data.value);
                       // setInputValue(introValueArr[i + 1]);
                   } else if (mouseStartPos > mouseEndPos) {
                       // moveBlock.style.left = (divWidth - moveBlockWidth / 2) + "px";
                       // console.log(i);
                       setSlideShower(introValueArr[i]);
                       setInputValue(introValueArr[i]);
                   }
                   ...
                   if (mouseStartPos < mouseEndPos) {
                       ...
                       setSlideShower(datamax);
                       setInputValue(datamax);
                   } else if (mouseStartPos > mouseEndPos) {
                       // moveBlock.style.left = (divWidth - moveBlockWidth / 2) + "px";
                       setSlideShower(introValueArr[introValueArr.length - 1]);
                       setInputValue(introValueArr[introValueArr.length - 1]);
                   }
               } else if (value < introValueArr[0] && value >= datamin) {
                   if (mouseStartPos < mouseEndPos) {
                       setSlideShower(introValueArr[0]);
                       setInputValue(introValueArr[0]);
                       // moveBlock.style.left = (divWidthOne - moveBlockWidth / 2) + "px";
                   } else if (mouseStartPos > mouseEndPos) {
                       setSlideShower(datamin);
                       setInputValue(datamin);
