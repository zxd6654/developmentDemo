...
;!function (window, undefined) {
    ...
    var isLayui = window.layui && layui.define, scope,
        $, win, ready = {
    ...
    Class.pt.creat = function () {
        ...
        // if (Object.prototype.toString.call(config.parameter) === "[object Object]") {
        //     var iframeWin = window[that.layero.find('iframe')[0]['name']];
        //     iframeWin.parameter = config.parameter;
        // }
    };
    ...
    Class.pt.callback = function () {
        ...
        if (config.success) {
            if (config.type == 2) {
                layero.find('iframe').on('load', function () {
                    config.success(config.parameter, layero, that.index);
                });
            } else {
                config.success(config.parameter, layero, that.index);
            }
        ...
        layero.find('.' + doms[6]).children('a').on('click', function () {
            ...
            if (index === 0) {
                if (config.yes) {
                    config.yes(layer.callback, that.index, layero)
                } else if (config['btn1']) {
        ...
        function cancel() {
            var close = config.cancel && config.cancel(layer.callback, that.index, layero);
            close === false || layer.close(that.index);
        }
    ...
    layer.apply = function () {
        if (scope.config.yes) {
            scope.config.yes(layer.callback, scope.index, scope.layero);
            layer.close(scope.config.index);
        }
    };

    layer.cancel = function () {
        var close = scope.config.cancel && scope.config.cancel(layer.callback, scope.index, scope.layero);
